/**
 * Bright Sparx Electrical & Security Services — website enquiry backend
 * ---------------------------------------------------------------------
 ...
 * turns them into GETs on the way, which loses e.postData. doPost is kept for
 * a JSON body in case the front end ever changes.
 */

/* ================= CONFIG ================= */

// Your own inbox while building and testing. Only change to the client's
// address (info@brightsparxelectrical.com.au) after sign-off, then redeploy
// a NEW VERSION of the deployment.
var NOTIFY_TO   = 'pavlegosic9@gmail.com';
var SENDER_NAME = 'Bright Sparx Website';
var BRAND       = 'Bright Sparx';
...
/* ================= PALETTE =================
   Pulled from the site's styles.css.
     --accent #00AEEF   --dark #0B1220   --light #EAF1F4
   #00AEEF is only 2.53:1 on white, so it is used for the decorative rule and
   the panel edge only. Text and button backgrounds use the deeper #00719B
   (5.48:1 on white) so everything passes AA. */

var C = {
  page:      '#EFF4F7', // soft tint behind the card, from --light
  card:      '#FFFFFF', // the card itself
  panel:     '#F5F8FA', // the message panel
  line:      '#E1E7EB', // hairlines
  brand:     '#00AEEF', // decorative only: header rule, panel edge
  brandInk:  '#00719B', // brand colour used as text
  brandDark: '#00719B', // primary button background
  ink:       '#0B1220', // headings and values, from --dark
  body:      '#3C4046', // message copy
  muted:     '#666B73', // labels, footer
...
// Matches the name attributes on contact.html and the payload in main.js.
...
function saveRow_(d) {
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(['Received', 'Name', 'Suburb', 'Phone', 'Email',
                     'Service', 'Property', 'Timing', 'Message', 'Page']);
  ...
  sheet.appendRow([
    new Date(),
    d.name || '', d.suburb || '', d.phone || '', d.email || '',
    d.service || '', d.property || '', d.timing || '',
    d.message || '', d.page || ''
  ]);
}
...
function buildSubject_(d) {
  ...
  // No company field on this form; the suburb is what makes a lead triageable.
  var tail = d.suburb ? ' (' + d.suburb + ')' : '';
...
function buildHtml_(d) {
  ...
  // A button only appears when the value is actually usable. People type "n/a".
...
'<tr><td align="center" style="padding:32px 12px 44px;">' +
  ...
  '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" ' +
    ...
    '<tr><td class="pad" bgcolor="' + C.card + '" style="background-color:' + C.card + ' !important;' +
      ...
      '<div style="color:' + C.brandInk + ' !important;font:700 11px/1.2 ' + SANS + ';letter-spacing:.2em;' +
      ...
      '<div style="color:' + C.muted + ' !important;font:400 15px/1.5 ' + SANS + ';padding-top:10px;">' +
        esc_(name) + (d.suburb ? ' &middot; ' + esc_(d.suburb) : '') + '</div>' +
    '</td></tr>' +
